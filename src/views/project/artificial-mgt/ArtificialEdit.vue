<template>
  <a-form ref="formRef" :model="formState" :rules="rules" :label-col="labelCol" :wrapper-col="wrapperCol">
    <a-form-item ref="name" label="子元名称" name="name">
...
import { updateArtificial } from "../../../assets/js/api_v2";
export default {
  ...
  setup(props, context) {
    ...
    var length = formState.value["url"].length;
    formState.value["url"] = formState.value["url"].substring(0, length - postfix.value.length);
    const onSubmit = () => {
      formRef.value
        .validate()
        .then((val) => {
          val.isChecked = val.isChecked == true ? 1 : 0;
          val["projectId"] = formState.value["projectId"];
          val["id"] = formState.value["id"];
          val["url"] = `${val.url}${postfix.value}`;
          updateArtificial(val)
            .then(() => {
